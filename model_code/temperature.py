"""
Temperature Solver for Energy Equation

Solves the steady-state energy equation with convection and diffusion:
∇·(ρvcp T) = ∇·(k∇T)

Simplified form (neglecting viscous dissipation and pressure work)
appropriate for low-Mach number flows in LPCVD reactors.
"""
...
class TemperatureSolver():
    """
    Solves energy equation for temperature field.
    
    Discretizes convection-diffusion equation:
    ∂(ρT)/∂t + ∇·(ρvT) = ∇·(k/cp ∇T)
    ...
    def solve_temperature(self, vr, vz, rho, tc, T):
        ...
        Discretized equation per cell:
        a_P*T_P = a_E*T_E + a_W*T_W + a_N*T_N + a_S*T_S
        
        where coefficients include both convection and diffusion:
        a_nb = D_nb + max(0, ±m_dot_nb)
        D_nb = k*A/Δ (thermal diffusion)
        
        Args:
            vr: Radial velocity (nr+1, nz)
            vz: Axial velocity (nr, nz+1)
            rho: Density (nr, nz)
            tc: Thermal conductivity (nr, nz)
            T: Temperature (nr, nz)
        
        Returns:
            T: Updated temperature field
        """
        nr, nz = self.nr, self.nz
        
        # Gauss-Seidel iterations with under-relaxation
        for outer_iter in range(20):
            ...
            for i in range(1, nr - 1):
                for j in range(1, nz - 1):
                    ...
                    # === GRID GEOMETRY ===
                    # Cell P sits at (r_centers[i], z_centers[j])
                    ...
                    # === FACE AREAS (per radian in axisymmetric) ===
                    r_e = r + 0.5 * dr_e
                    r_w = r - 0.5 * dr_w
                    A_e = r_e * dz_p
                    A_w = r_w * dz_p
                    ...
                    # Use current density and velocity fields
                    m_dot_e = rho[i, j] * vr[i+1, j] * A_e
                    m_dot_w = rho[i, j] * vr[i, j] * A_w
                    m_dot_n = rho[i, j] * vz[i, j+1] * A_n
                    m_dot_s = rho[i, j] * vz[i, j] * A_s

                    # === THERMAL CONDUCTIVITIES at faces ===
                    # Interpolate from adjacent cell centers
                    ...
                    # D = k*A/Δ for thermal conduction
                    D_e = tc_e * A_e / dr_e
                    D_w = tc_w * A_w / dr_w
                    D_n = tc_n * A_n / dz_n
                    D_s = tc_s * A_s / dz_s
                    ...
                    # Upwind scheme: add max(0, inflow) to diffusion
    ...
    def solve(self, vr, vz, rho, tc, T):
        """
        Main solver interface for temperature equation.
