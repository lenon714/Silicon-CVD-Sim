...
def run_diagnostics(solver):
    """Run after a few iterations to check algorithm consistency"""
    ...
    # Inlet mass flux (top, inside pipe)
    inlet_flux = 0.0
    for i in range(nr):
        r = grid.r_centers[i]
        if r < solver.config.pipe_radius:
            dr = grid.dr[i]
            # vz at top face (j = nz), negative = inflow
            inlet_flux += solver.rho[i, -1] * (-solver.vz[i, -1]) * r * dr
    
    # Outlet mass flux (bottom, outside wafer)
    outlet_flux = 0.0
    for i in range(nr):
        r = grid.r_centers[i]
        if r >= solver.config.wafer_radius:
            dr = grid.dr[i]
            # vz at bottom face (j = 0), negative = outflow
    ...
    # Pick a cell away from boundaries
    ...
    # 8. ITERATION-TO-ITERATION CHANGES
    print("\n8. RECOMMENDATIONS")
    print("-" * 40)
...
def check_velocity_correction(solver):
    """Verify velocity correction is applied correctly"""
    print("\n" + "="*70)
    ...
    # Store pre-correction velocities
    vr_before = solver.vr.copy()
    vz_before = solver.vz.copy()
    
    # Manually compute what the correction should be
...
def check_pressure_equation_coefficients(solver):
    """Verify pressure equation coefficient computation"""
    print("\n" + "="*70)
